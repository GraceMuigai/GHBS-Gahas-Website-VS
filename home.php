<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Home</title>

    <!--swipper css link -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@8/swiper-bundle.min.css" />

    <!--font awesome cdn link -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css">

<!--font google-->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Andika:ital@0;1&family=Lato:ital@0;1&family=Roboto:ital,wght@0,400;1,500&display=swap" rel="stylesheet">

<!--custom css file link-->
<link rel="stylesheet" href="css/Style.css">


</head>
<body>

    <!-- header section starts -->
    <section class="header">
        <a href="home.php" class="logo">Gahas Braiding Creativity</a>

        <nav class="navbar">
            <a href="home.php">Home</a>
            <a href="services.php">Services</a>
            <a href="../GHBS Gahas Website VS/buy products/site_products.php">Products</a>
            <a href="about.php">About</a>
            <a href="book.php">Book</a>
        </nav>
        <div id="menu-btn" class="fas fa-bars">
            
        </div>
    </section>
    <!-- header section ends -->


    <!-- home section starts -->
    
    <section class="home">

        <div class="swiper home-slider">
            
            <div class="swiper-wrapper">
                
                <div class="swiper-slide slide" style="background:url(images/home-slider-gahas.jpg) no-repeat">

                    <div class="content">
                        <span>explore, hair care, consultation</span>
                        <h3>See your hair grow</h3>
                        <a href="services.php" class="btn">Know More</a>
                    </div>
                </div>

                <div class="swiper-slide slide" style="background:url(images/home-slider-locs.png) no-repeat">

                    <div class="content">
                        <span>explore, hair care, consultation</span>
                        <h3>Braid New Hairstyles</h3>
                        <a href="services.php" class="btn">Know More</a>
                    </div>
                </div>

                <div class="swiper-slide slide" style="background:url(images/home-slider-afro.png) no-repeat">

                    <div class="content">
                        <span>explore, hair care, consultation</span>
                        <h3>Hair Growth, Smile</h3>
                        <a href="services.php" class="btn">Know More</a>
                    </div>
                </div>

                <div class="swiper-slide slide" style="background:url(images/home-slider-oil.jpg) no-repeat">

                    <div class="content">
                        <span>explore, hair care, consultation</span>
                        <h3>Proud of Your Hair</h3>
                        <a href="../GHBS Gahas Website VS/buy products/site_products.php" class="btn">Shop Products</a>
                    </div>
                </div>

            </div>

            <div class="swiper-button-next"></div>
            <div class="swiper-button-prev"></div>

        </div>

    </section>

    <!-- home section ends -->







    <!-- packages section starts -->
    <section class="packages">

        <h1 class="heading-title"> our packages </h1>

        <div class="box-container">

            <div class="box">
               <img src="images/icon-curly-hair.png" alt="">
               <h3>hair care</h3> 
            </div>

            <div class="box">
                <img src="images/icon-afro-beauty.png" alt="">
                <h3>consultation</h3> 
             </div>

             <div class="box">
                <img src="images/icon-combs.png" alt="">
                <h3>Plaiting</h3> 
             </div>

        </div>

    </section>

    <!-- packages section ends -->

    <!-- product category starts -->

    <section class="home-category">

        <h1 class="heading-title">Category of Products</h1>
            
        <div class="swiper category-slider">
            <div class="swiper-wrapper">

                <a href="buy products/category.php?category=kinky" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>kinky</h3>
                </a>

                <a href="buy products/category.php?category=locs" class="swiper-slide slide">
                    <img src="images/icon-1.png" alt="">
                    <h3>locs</h3>
                </a>

                <a href="buy products/category.php?category=braids" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>sell hair products</h3>
                </a>

                <a href="buy products/category.php?category=hair-bands" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>sell hair products</h3>
                </a>

                <a href="buy products/category.php?category=aloe-vera" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>aloe vera</h3>
                </a>

                <a href="buy products/category.php?category=coconut-oil" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>coconut oil</h3>
                </a>

                <a href="buy products/category.php?category=fluffy-twist" class="swiper-slide slide">
                    <img src="images/icon-wavy.png" alt="">
                    <h3>fluffy twist</h3>
                </a>
            </div>

            <div class="swiper-pagination"></div>

        </div>

        

    </section>



    <!-- product category ends -->





    <!-- home about section starts -->

    <section class="home-about">

        <div class="image">
            <img src="images/about-lydia-locs.png" alt="">
        </div>

        <div class="content">
            <h3>about us</h3>

            <p><centre><i>It is time to take action and care</i></centre></p>
            <p>
                We have created a hair caring and braiding website. We do nice braiding any styles we are the best. We have professionals 
                in different sectors like growing hair, fashioning and styling hair. This is to help you to grow a healthy hair and care for it more. In the end you 
                will see a great change in your final results.<br>
                <centre>amazing!!</centre>
            </p>
            <a href="about.php" class="btn">read more</a>
        </div>

    </section>


    <!-- home about section ends -->


    <!-- home services section starts -->

    <section class="home-services">

        <h1 class="heading-title">Our Services</h1>

        <div class="box-container">

            <div class="box">
                <div class="image">
                    <img src="images/home-services-lydia-locs-back.png" alt="">
                </div>
                <div class="content">
                    <h3>Butterfly Locs</h3>
                    <p>Beautifully made locs which are light weight and free to style.</p>
                    <a href="book.php" class="btn">Book Now</a>
                </div>
            </div>

            <div class="box">
                <div class="image">
                    <img src="images/home-services-eunice-natural-braids.png" alt="">
                </div>
                <div class="content">
                    <h3>Natural Braids</h3>
                    <p>Awesomwe braids that are natural and proudly walk with them. The African Beauty!!</p>
                    <a href="book.php" class="btn">Book Now</a>
                </div>
            </div>

            <div class="box">
                <div class="image">
                    <img src="images/home-services-mum-braids.png" alt="">
                </div>
                <div class="content">
                    <h3>Braids</h3>
                    <p>Long Lasting braids that have added extension. Comfortable for any age group.</p>
                    <a href="book.php" class="btn">Book Now</a>
                </div>
            </div>


        </div>

        <div class="load-more"><a href="services.php" class="btn"> Load More </a></div>

    </section>

    <!-- home services section ends -->
    

    






    <!-- footer section starts -->
    <section class="footer">

        <div class="box-container">
            
            <div class="box">
                <h3>Quick Links</h3>
                <a href="home.php"> <i class="fas fa-angle-right"></i> Home</a>
                <a href="services.php"> <i class="fas fa-angle-right"></i> Services</a>
                <a href="../GHBS Gahas Website VS/buy products/site_products.php"> <i class="fas fa-angle-right"></i> Products</a>
                <a href="about.php"> <i class="fas fa-angle-right"></i> About</a>
                <a href="book.php"> <i class="fas fa-angle-right"></i> Book</a>
            </div>

            <div class="box">
                <h3>Extra Links</h3>
                <a href="#"> <i class="fas fa-angle-right"></i> Ask questions</a>
                <a href="#"> <i class="fas fa-angle-right"></i> About us</a>
                <a href="#"> <i class="fas fa-angle-right"></i> Privacy Policy</a>
                <a href="#"> <i class="fas fa-angle-right"></i> Terms of Use</a>
            </div>

            <div class="box">
                <h3>Contact Info</h3>
                <a href="#"> <i class="fas fa-phone"></i> +254 740 403756 </a>
                <a href="#"> <i class="fas fa-phone"></i> +254 772 868174 </a>
                <a href="#"> <i class="fas fa-envelope"></i> gahasbraidingcreativity@gmail.com </a>
                <a href="#"> <i class="fas fa-map"></i> kenyatta road, Juja </a>
            </div>

            <div class="box">
                <h3>Follow Us</h3>
                <a href="#"> <i class="fab fa-facebook-f"></i> facebook </a>
                <a href="#"> <i class="fab fa-twitter"></i> twitter </a>
                <a href="#"> <i class="fab fa-instagram"></i> instagram </a>
                <a href="#"> <i class="fab fa-linkedin"></i> linkedin </a>
            </div>

        </div>

        <div class="credit"> created by <span>Grace Muigai aka Gahas</span>  |  all rights reserved! </div>
    </section>
    <!-- footer section ends -->

  


    <!-- swiper js link -->
    <script src="https://cdn.jsdelivr.net/npm/swiper@8/swiper-bundle.min.js"></script>

    <!-- custom js file link -->
     <script src="js/Script.js"></script>
    
</body>
</html>